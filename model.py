...
defaultMinimum = 10
defaultMidum2 = 20  # Corrected spelling to defaultMidum
defaultMidum3 = 30  # Corrected spelling to defaultMidum1
defaultMaximum = 45
...
speeds = {'car':1.8, 'bus':1.7, 'truck':1.4, 'rickshaw':2, 'bike':2.3}  # average speeds of vehicles
...
class Vehicle(pygame.sprite.Sprite):
    def _init_(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        if(direction=='right'):
            ...
            stops[direction][lane] += temp
        simulation.add(self)
        def calculate_remaining_vehicles(self):
            # Example: Calculate the number of vehicles remaining in the current lane
            if self.direction == 'right':
                remaining_vehicles = len([v for v in vehicles[self.direction][self.lane] if v.crossed == 0])
...
def setTime():
    ...
    # Count vehicles waiting at the signal
        ...
        for j in range(len(vehicles[directionNumbers[nextGreen]][i])):
            ...
            if vehicle.crossed == 0:
                ...
                if vclass == 'car':
                    ...
                    noOfRickshaws += 1
            # Calculate remaining vehicles
            remainingVehicles = sum(vehicle.calculate_remaining_vehicles() for direction_list in vehicles.values() for lane_list in direction_list for vehicle in lane_list)

            # Save cycle data including remaining vehicles
            saveCycleData(noOfCars, noOfBuses, noOfTrucks, noOfRickshaws, noOfBikes, remainingVehicles)

    

    # Existing greenTime calculation
    greenTime = math.ceil(((noOfCars * 2) + (noOfRickshaws * 2) + (noOfBuses * 3) + (noOfTrucks * 3) + (noOfBikes * 1)) / (noOfLanes))
    ...
    if time_seconds <= 20:
        ...
        greenTime = defaultMidum2
    elif 40 < time_seconds <= 60:
        greenTime = defaultMidum3
    elif time_seconds > 60:
        greenTime = defaultMaximum

    

def saveCycleData(noOfCars, noOfBuses, noOfTrucks, noOfRickshaws, noOfBikes, remainingVehicles):
    ...
    data = {
        ...
        'Bikes': [noOfBikes],
        'Remaining Vehicles': [remainingVehicles]  # Added column for remaining vehicles
    }
...
class Main:
    ...
    while True:
        ...
        # Create a DataFrame to store vehicle counts
